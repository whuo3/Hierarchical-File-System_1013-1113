	...
	)
{
	...
	struct	lfiblk  fbptr;	/* ptr to file's entry in the	*/
					/*  in-memory directory		*/
	...
	lfibget(Lf_data.lf_dskdev, lfptr->lffilptr, &fbptr);
	if (lfptr->lfpos > fbptr.ld_size) {
		signal(lfptr->lfmutex);
		return SYSERR;
	}
	...
	if (lfptr->lfpos >= fbptr.ld_size) {
		//ldptr->ld_size++;
		//Refrech the first iblock of file
		fbptr.ld_size++;
		lfibput(Lf_data.lf_dskdev, lfptr->lffilptr, &fbptr);
	}

	/* Place byte in buffer and mark buffer "dirty" */

	*lfptr->lfbyte++ = ch;
	lfptr->ld_size = fbptr.ld_size;
	lfptr->lfpos++;
